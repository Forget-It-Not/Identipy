...
def build_muscle (result_path):
    ...
    Input:
        - result_path: ruta donde está el archivo input y donde dejar el resultado

    Output: genera el archivo newick del árbol filogenético. El return se silencia a 'None'
    '''
    #Se abre devnull para silenciar el mensaje de stderr que genera muscle
    #(que no es un error en realidad)
    with open(os.devnull,'w') as devnull:
        call(['muscle', '-in',result_path+'/MultifaFiltered.fasta', '-out', result_path+'/MuscleAlign'], stderr=devnull)
    return None
...
def plot_muscle (result_path):
    ...
    with open(os.devnull, 'w') as devnull:
        call(['muscle', '-maketree', '-in', result_path+'/MuscleAlign', '-out', result_path+'/MuscleTree.nw'], stderr=devnull)
