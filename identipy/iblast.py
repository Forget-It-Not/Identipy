...
def blastp_hits (query_path, result_path, eval, cov_t, iden_t):
    ...
    with open('blast_temporal','w') as blast_file:
        call(['blastp','-query',query_path,'-subject', 'MultifaDB.fasta', '-evalue', str(eval), '-outfmt', '6 qseqid sseqid qcovs pident sseq'], stdout=blast_file)
    ...
    with open('blast_temporal','r') as blast_file:
        ...
        for line in lines:
            ...
            #Si pasan los filtros de cov e iden se guardan en la lista de hits con la que se filtra luego
...
def blast_plot (blast_result):
    ...
    '''
    #Se inicializan variables cov e iden donde se guardan los valores respectivos
    cov = []
    iden = []
